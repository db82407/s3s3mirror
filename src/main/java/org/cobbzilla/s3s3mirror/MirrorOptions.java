...
public class MirrorOptions implements AWSCredentials {
    ...
    public static final String USAGE_PREFIX_FILE = "Read prefixes from specified file.";
    public static final String OPT_PREFIX_FILE = "-F";
    public static final String LONGOPT_PREFIX_FILE = "--prefix-file";
    @Option(name = OPT_PREFIX_FILE, aliases = LONGOPT_PREFIX_FILE, usage = USAGE_PREFIX_FILE)
    @Getter
    @Setter
    private String prefixFile = null;

    public boolean hasPrefixFile() {
        return prefixFile != null && prefixFile.length() > 0;
    }
    ...
    @Argument(index = 0, required = true, metaVar = "SourceBucket", usage = "[[s3:]//]source[/prefix]")
    @Getter
    @Setter
    private String source;
    @Argument(index = 1, required = false, metaVar = "DestBucket", usage = "[[s3:|gs:]//]dest[/prefix]")
    @Getter
    @Setter
    ...
    @Option(name = "-?", aliases = "--help", usage = "Show this help")
    @Getter
    @Setter
    ...
    @Option(name = "-f", aliases = "--fix-etags", usage = "Fix multi-part Etags that are not MD5 checksums.")
    @Getter
    @Setter
    private boolean fixEtags = false;

    public boolean fixEtags() {
        return fixEtags;
    }

    @Option(name = "-M", aliases = "--check-meta", usage = "Copy if meta-data differs.")
    @Getter
    @Setter
    ...
    public void initDerivedFields() {
        ...
        if (slashPos == -1) {
            ...
            if (hasPrefix() || hasPrefixFile())
                throw new IllegalArgumentException("Cannot use " + LONGOPT_PREFIX + " or " + LONGOPT_PREFIX_FILE +
                        " and source that includes a prefix");
            prefix = scrubbed.substring(slashPos + 1);
        }

        if (fixEtags()) {
            if (destination != null) {
                throw new IllegalArgumentException("Cannot specify \"DestBucket\" and --fix-etags");
            }
            destination = "fix-etags";
        } else if (destination == null) {
            throw new IllegalArgumentException("Argument \"DestBucket\" is required");
        }

        // preserve dest scheme for non-S3 copy tasks
        ...
        if (host == null) {
            ...
            path = "";
        } else if (scheme == null || scheme.equals("s3")) {
            destinationBucket = host;
        } else {
            destinationBucket = scheme + "://" + host;
        }

        if (!path.isEmpty()) {
            if (hasDestPrefix() || hasPrefixFile())
                throw new IllegalArgumentException("Cannot use " + LONGOPT_DEST_PREFIX + " or " + LONGOPT_PREFIX_FILE +
                        " and a destination that includes a prefix");
